...
library(data.table); library(truncdist)

# ------------- VALORES TABLA --------------------------- ----

param <- data.frame(psi = 1977,            # Year inflection point diffusion of contraception
                    upsilon = 0.39,        # Maximum Risk Unplanned births 
                    rho = 0.037,           # minimum risk of unplanned birth
                    r = 0.23,              # Speed of diffusion contraception
                    eta = 0.57,            # Max effect work  
                    xi = 4.3,              # years after end of education for family formation
                    D_0 = 0.8,             # initial value desired family size (Beta in paper)
                    delta = 0.009,         # delta D
                    tau = 25.1,            # effect of edu on intention LISTO
                    epsilon = 0.53,        # rate effect education
                    alpha = 0.07)          # difference in contraceptive use by edu

fix_p <- list(lambda = 2.5e-08,            # rate decrease penalty intention after birth
              sd_lnrm = 0.16,              # stdrd dev lognorm -- FLAG (NO ESTA EN LA TABLA) 
              ...
              theta = 0.1)                 # scale of truncated Gamma (D) -- FLAG (NO ESTA EN LA TABLA) 

# ------------- INPUTS MODELO --------------------------- ----

ini_c = 2500
country = "NO"
pop = "NO"            # reference population
iniY = 1910           # initial year of the simulations 
endY = 2019           # final year of the simulation



psi = 1977            # Year inflection point diffusion of contraception
upsilon = 0.39        # Maximum Risk Unplanned births 
rho = 0.037           # minimum risk of unplanned birth
r = 0.23              # Speed of diffusion contraception
eta = 0.57            # Max effect work  
...
D_0 = 0.8             # initial value desired family size (Beta in paper)
delta = 0.009         # delta D
tau = 25.1            # effect of edu on intention 
epsilon = 0.53        # rate effect education
alpha = 0.07          # difference in contraceptive use by edu
...
end_mau = 19.7
ini_mau = 32
u = c(seq(0.22, 0.09, length.out = 1938-iniY),   # single probability:
      seq(0.09, 0.16, length.out = (endY+1)-1938))
...
comfert <- function(seed_val, param, parallel_run = F, cluster = F){
  ...
  while (time < endTime){
    ...
    if (nextEvent == "evalBirth") {
      ...
      pop[rid, intentBixrth := baseline - edu_int(y = edu_y) * activity -
