  ...
  if(element=="asfr"){
    ...
    for (i in 1:nrow(accepted)){
      ...
      aux_sim <- get_sim(ruta, iniY, endY, nsim = 2,
                         obs_set, asfr = T,  all_sim = F) # to extract the 2 full_results of the corresponding paramset folder
      
      colnames(obs_df_asfr) <- colnames(obs_set$obs_asfr)
      colnames(sim_df_asfr) <- colnames(aux_sim$sim_asfr$asfr)
      
      obs_df_asfr <- rbind(obs_df_asfr,obs_set$obs_asfr)
