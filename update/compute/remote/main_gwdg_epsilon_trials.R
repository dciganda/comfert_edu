...
priors <- data.frame(psi = c(1972, 1978),           # Year inflection point diffusion of contraception.
                     upsilon = c(0.1, 0.8),       # Maximum Risk Unplanned births
                     rho = c(0.01, 0.050),         # minimum risk of unplanned birth
                     r = c(0.1, 0.3),             # Speed of diffusion contraception
                     eta = c(0.3, 0.7),             # Max effect work
                     xi = c(3, 6),                  # years after end of education for family formation
                     D_0 = c(2.3, 2.6),               # initial value desired family size
                     delta = c(0.01, 0.1),           # delta D
                     tau = c(12, 19),               # effect of edu on intention 
                     alpha = c(0.05, 0.14)           # difference in contraceptive use by edu
)
...
cluster_path <- "U:/cloud/comfert/comfert_edu/update/"
...
saveRDS(t_out$simDat, file.path(st_dir, "sim_trajectories_45_0.02.rds"))
...
# simulated data 
sim_dat <- readRDS(file.path(st_dir, "sim_trajectories_45_0.02.rds"))
...
# Plot
source(file.path("..","analysis","ploting_new.R"))

plot_obs(models_scnd, level = "SEC", save = F)
plot_obs(models_scnd, level = "TER", save = F)

plot_obs_sim(models_scnd, level = "SEC", ylim = c(0.4,1.6), save = F)
plot_obs_sim(models_scnd, level = "TER", ylim = c(0.4,1.6), save = F)
