...
weights <- c(asfr = .0,
             ...
             unwanted = 0.0,
             desired = .0,
             ccf_edu = 1) # weights for the computation of the MSE
...
priors <- data.frame(psi = c(1972, 1979),           # Year inflection point diffusion of contraception.
                     ...
                     tau = c(20, 30),               # effect of edu on intention 
                     epsilon = c(0.01, 0.1),         # rate effect education
                     alpha = c(0.03, 0.14)          # difference in contraceptive use by edu
)
...
min_mse <- post[order(post$mse),]$mse[5] # get min mse
